...
!function ($) {
    ...
    var createControls = function (that, header) {
        ...
        $.each(that.columns, function (i, column) {
            ...
            if (column.filterData !== undefined && column.filterData.toLowerCase() !== 'column' &&
                column.filterControl !== undefined && column.filterControl.toLowerCase() === 'select') {
                ...
                switch (filterDataType) {
                    ...
                    case 'var':
                        ...
                        break;
                    case 'str':
                        var storeParts = filterDataSource.split('-');
                        var storeData = Stores.getStore(storeParts[0], storeParts[1]).data;
                        for (var key in storeData) {
                            addOptionToSelectControl(selectControl, key, storeData[key]);
                        }
                        break;
                }
            }
    ...
    BootstrapTable.prototype.onColumnSearch = function (event) {
        ...
        var column = this.columns.find(function(a) { return a.field === $field })
        
        if(column.filterData.substring(0, 3).toLowerCase() === 'str'
            && column.filterControl.toLowerCase() === 'input'
            && text.length > 0) {
            var storeParts = column.filterData.split('-');
            var storeData = Stores.getStore(storeParts[1], storeParts[2]).data;
            var keys = [];
